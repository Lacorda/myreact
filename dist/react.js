(function (win) {
  /**
   * ReactTextComponent
   * @param text {string} 暂时只实现文本类型节点
   * @constructor _currentElement 当前元素; _reactNodeId 标识Id
   * @function {Function} mountComponent 渲染DOM
   */
  function ReactTextComponent(text) {
    this._currentElement = '' + text;
    this._reactNodeId = null;
  }

  ReactTextComponent.prototype.mountComponent = function (rootId) {
    this._reactNodeId = rootId;
    return '<span data-reactid="' + rootId + '">' + this._currentElement + '</span>';
  };

  ReactTextComponent.prototype.receiveComponent = function (nextText) {
  ...
  ReactDOMComponent.prototype.mountComponent = function (rootID) {
    ...
    for (var propKey in props) {
      ...
      if (/^on[A-Za-z]/.test(propKey)) {
        ...
        //这里直接使用jquery事件机制，实际上react有自己的事件处理方式，比较复杂
...
//普通的children是一个数组，此方法把它转换成一个map,key就是element的key,如果是text节点或者element创建时并没有传入key,就直接用在数组里的index标识
  ...
  ReactCompositeComponent.prototype.mountComponent = function (rootID) {
    this._rootNodeID = rootID;
    //拿到当前元素对应的属性值 {name: "John"}
    var publicProps = this._currentElement.props;
    //拿到对应的ReactClass
    /** type = React.createClass(@return 继承ReactClass的Constructor)
  ...
  function instantiateReactComponent(node) {
     ...
     * node = 'Hello World' => typeof node = 'string'
     */
     ...
     * node = element(React.createElement,return ReactElement) => typeof node = 'object'
     * node.type = 'div' => typeof node.type = 'string'
     */
     ...
     * node = React.createElement(return ReactElement) => typeof node = 'object'
     * node.type = HelloMessage(React.createClass) => typeof node.type = 'function'
     */
  ...
  /**
   *
   * @param type {string} label's name
   * @param key {string} label's key for unique mark
   * @param props {object} label's props include the children
   * @constructor
   */
  ...
  var React = {
     ...
     *
     * @param type {string} label's name
     * @param config {object} label's props
     * @param children {string} label's children (暂定为只有一层的文本节点)
     * @return {ReactElement} React's virtual Dom
     */
    ...
    render: function (element, container) {
      ...
      $(container).html(componentInstance.mountComponent(React.nextReactRootID++));
